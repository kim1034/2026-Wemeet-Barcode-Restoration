...
| 샤드 수 | **`L` 1 / `M` 5 / `H` 9 = 15** | 설계 §4. 2026-09-15 정정 — 프로토타입 값 `L` 2 / `M` 6 / `H` 7 을 Task 6·7 의 재파일럿 측정 속도로 다시 배분했다. 합 15 는 유지 |
...
렌더 상한은 재파일럿(682a3ab) 값 `L 182,926 / M 218,446 / H 74,504`, `τ = 0.0741`,
샤드 `L1/M5/H9`, 시드 42, 쿼터 `5000/3500/1500` 을 그대로 썼다.

| 버킷 | 렌더 | 목표 | 불가·기하 | 1차 성공 | 부족분 | 샤드 `hit_cap` |
|---|---|---|---|---|---|---|
| `L` | 126,983 | 5,000 | 3,500 | 1,500 | 없음 | 1개 전부 `false` |
| `M` | 163,453 | 5,000 | 3,500 | 1,500 | 없음 | 5개 전부 `false` |
| `H` |  48,147 | 5,000 | 3,500 | 1,500 | 없음 | 9개 전부 `false` |

세 버킷 모두 쿼터를 정확히 채웠고 샤드 하나도 상한에 닿지 않았다. 부족분이 없으므로
분포 탓인지 쪼갠 탓인지를 가를 일이 없다. `H` 가 렌더 수가 제일 적은 것은 목표 구간
출현율이 10.07% 로 제일 높기 때문이다(예측 49,652 대비 실측 48,147).

실현된 preset 비율 (목표 구간) — 뽑을 때는 30/30/30/10 이다:

| 버킷 | crease | sine | octave | cylinder |
|---|---|---|---|---|
| `L` | 0.4270 | 0.2620 | 0.2138 | 0.0972 |
| `M` | 0.2330 | 0.2504 | 0.4442 | 0.0724 |
| `H` | 0.1644 | 0.2926 | 0.4896 | 0.0534 |

해상도가 올라갈수록 `crease` 가 빠지고 `octave` 가 그 자리를 메운다(0.427→0.164 /
0.214→0.490). 저해상도에서는 접힘이 목표 구간을 만들지만 고해상도에선 너무 쉽게 풀려
1차 성공으로 새는 것으로 읽힌다. `cylinder` 는 세 버킷 모두 설계치(0.10) 근처다.
설계 §7 대로 보고만 하고 보정하지 않는다.

용량:

| | 원본 | gzip |
|---|---|---|
| `train.L.jsonl` |  93.3 MB | 29.7 MB |
| `train.M.jsonl` | 120.2 MB | 38.4 MB |
| `train.H.jsonl` |  35.4 MB | 11.4 MB |
| 평가 레시피 3벌 |  23.5 MB | — |
| **레시피 합계** | **272.5 MB** | **79.5 MB** |
| 평가 세트(구운 PNG+NPZ) | 157.8 MB | — |

설계 §1 이 예측한 222~334 MiB 안이다. HF 에는 Task 9 대로 **gzip 해서** 올린다
(`synthetic/v1/recipes/*.jsonl.gz`) — `datasets` 가 gzip 을 투명하게 읽으므로 손실이 없고 79.5 MB 로 줄어든다.

다음 세대 `τ` 후보(이번 생성엔 쓰이지 않았다): 생성물의 목표 구간 포화율 95 백분위수가
`L 0.1516 / M 0.1035 / H 0.0426` 으로 나왔다. 버킷별로 3.6배 벌어져 단일 값이 맞는지는
다음 세대에서 다시 볼 일이다.

**재현성**(설계 검증 #6): `L n=300 shards=2` 를 두 번 돌려 `_stats` 헤더를 제외한
300행의 sha256 이 같음을 확인했다 (`2e5b274e7a029ac2725f5f72533e72c975b2f2fda740e39d57d31ac2e7eb430c`).

**`code.dirty`**: `M`·`H` 의 stats 가 `dirty: true` 로 찍혔다. 앞서 끝난 버킷의
`data/stats/train.*.stats.json` 이 그 시점에 untracked 였기 때문이고, 생성 코드는 세 버킷
모두 `290e154` 로 동일하다. Task 7 이 기록한 `code_commit()` 의 알려진 노이즈다
— 추적 안 되는 파일과 생성 코드 변경을 구분하지 못한다.
