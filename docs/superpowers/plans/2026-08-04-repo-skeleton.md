...
이 계획을 실행하면서 위 내용과 달라진 점만 적는다. 위 본문은 고치지 않았다 — 무엇을 계획했고 무엇이 실제로 달랐는지가 정보다.

### 계획대로 된 것

Task 1~6 전부 완료. 커밋 6개. 빈 클론에서 `uv sync` → `pytest` 11 passed → `lint-imports` 4 kept → `ruff` 통과를 확인했다. 설치된 파이썬은 3.12.13.

### 1. `import-linter`는 없는 모듈을 오류 없이 받아들인다 (해결됨)

"달라지는 3가지" 2번의 불확실성이 해소됐다. `forbidden_modules` 에 존재하지 않는 `wemeet.sw.pipeline` / `wemeet.sw.server` 를 넣고 돌려보니 `4 kept, 0 broken`, exit 0 이었다. **따라서 설계 §4 원안 그대로 넣어뒀다.** 2단계에서 계약을 고칠 필요가 없다.

대신 새로운 주의사항이 생겼다 — 검증을 안 하므로 **오타를 내면 규칙이 조용히 무력화된다.** `pyproject.toml` 의 해당 계약에 주석으로 남겼다. "2단계로 넘기는 검증 2개"는 그래서 여전히 필수다.

### 2. `ruff format` 이 마크다운 안의 파이썬 블록까지 재작성한다 (계획에 없던 문제)

Task 2에서 `uv run ruff format .` 을 돌리자 설계 문서와 파트별 문서 **5개가 수정됐다.** ruff가 `.md` 안의 ` ```python ` 블록을 포맷하기 때문이다. 문서의 코드 예시는 1~2학년이 읽기 쉽도록 일부러 정렬해둔 것이 많아서 그게 다 뭉개졌다.

```diff
-    crop_bgr_uint8 = crop,      # 잘라낸 이미지. BGR 순서, 0~255
-    angle_deg_ccw  = 12.5,      # 몇 도 돌려서 세웠는지
+    crop_bgr_uint8=crop,  # 잘라낸 이미지. BGR 순서, 0~255
+    angle_deg_ccw=12.5,  # 몇 도 돌려서 세웠는지
```

더 큰 문제는 CI다. `ruff format --check .` 가 문서 때문에 실패하므로 **모든 PR이 머지 불가가 된다.** `CONTRIBUTING.md`가 팀원에게 `uv run ruff format .` 을 안내하고 있어서 더 나쁘다.

`pyproject.toml` 에 `extend-exclude = ["*.md"]` 를 넣어 해결했다. ruff가 보는 파일이 25개 → 7개(파이썬만)로 줄었다. 되돌린 문서 5개는 커밋되지 않았다.

**교훈**: 문서에 코드 블록이 많은 저장소에서 포매터를 처음 붙일 때는 `--check` 로 먼저 무엇이 걸리는지 보고, 그다음 포맷할 것.

### 3. 완료 판정 중 2개는 GitHub 설정이라 아직 미확인

- `main` 에 직접 push 거부 — 브랜치 보호를 켠 뒤 확인해야 한다
- 승인 없는 PR 머지 불가 — 같음

`docs/external/README.md` §1의 브랜치 보호 설정이 선행 조건이다.
