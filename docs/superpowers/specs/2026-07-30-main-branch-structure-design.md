...
> **Stage 2 정의와 `RestoredBarcode` 는 폐기됐다 (2026-08-04).**
> 대체 문서: [기하 추정·보정 파이프라인 설계](2026-08-04-geometry-pipeline-design.md)
>
> **무엇이 바뀌었나**: Stage 2가 "이미지 복원(생성)"에서 "기하 추정(예측) + 기하 보정(OpenCV 변환)"으로 바뀌었다. AI는 픽셀을 만들지 않고 **얼마나 휘었는지만** 예측하고, 실제 변형은 OpenCV `remap`이 한다. 단계가 3개 → 4개가 되었다.
>
> **왜**: 생성 모델이 만든 가짜 바코드는 체크섬을 우연히 통과할 수 있다(Code128 약 1/103). 틀린 번호가 조용히 통과하는 것은 못 읽는 것보다 나쁘다. 그리고 기하 추정은 합성으로 정답을 무한정 만들 수 있어 9월 데이터 위험이 크게 줄어든다.
>
> **아래 내용 중 유효한 것**: `DetectedBarcode` · `DecodeResult` · `PipelineResult` 정의, 실패와 예외 상황 정책, 로깅 정책, 이름에 단위를 박는 원칙. **폐기된 것**: `RestoredBarcode`, `restore()` 시그니처, 시간 예산 배분(새 배분은 대체 문서 §11).
>
> 지우지 않고 남긴다 — 11월 보고서에서 "왜 생성 복원을 쓰지 않았나"에 답할 근거다.
