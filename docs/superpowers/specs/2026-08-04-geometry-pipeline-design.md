...
> ⚠️ **2026-08-28 정정 — 이 절의 실측 조건은 모듈 폭 5.5px 가 아니다.**
>
> 재측정에서 이 절의 결과가 재현되지 않았다. 5.5px 에서는 원통 70°까지 보정 없이 읽힌다.
> `d_m0 = 2.3px` 에서 §8.1 의 "35° 이상 실패"가 정확히 재현되고, 그때 원본 폭이 355px 로
> 이 절이 적은 376×280 과 거의 같다. **"376×280, 모듈 5.5px" 는 서로 양립하지 않는다** —
> Code128 `WEMEET0001` 은 quiet zone 포함 165 모듈이므로 376px 이면 모듈이 2.3px 다.
>
> 따라서 §8.1·§8.2·§8.4 의 판정은 **모듈 폭 약 2.3px 조건**으로 읽어야 하고,
> §8.4 표의 "35°→4.5px" 류 숫자는 5.5px 를 가정해 계산한 것이라 실험 조건과 어긋난다.
> 자세한 재측정은 [2026-08-28 합성 스파이크](../../experiments/2026-08-28-synthesis-spike/README.md).
...
> **2026-08-28 정정 — 적분 방향.** 높이장 기울기에서 매핑을 유도할 때
> `s = np.cumsum(1/m)` 이다. `cumsum(m)` 이 아니다.
>
> `m = 1/√(1+z_x²) ≤ 1` 이므로 `cumsum(m) < x` 가 되어 **펴진 폭이 관측 폭보다 좁아진다** —
> 종이를 펴면 반드시 길어지므로 물리적으로 불가능한 결과다. 원통 해석해와 대조하면
> 정정판이 0.24px, 오류판이 35.1px 벗어난다. 규약은 이렇게 고정한다.
>
> | | 의미 |
> |---|---|
> | `x` | 관측 좌표 (사진에 찍힌 것). `z` 는 이 좌표 위의 높이 |
> | `s` | 펴진 좌표 |
> | `J = ds/dx = √(1+z_x²) ≥ 1` | 신장률 |
> | `m = dx/ds = 1/J ≤ 1` | 압축률. 관측 모듈 폭 = `d_m0 · m` |
>
> 초과분은 `np.clip(m, …)` 로 자르지 않는다. m 만 자르면 대응하는 z 가 없어져
> **음영·반사가 기하와 어긋난다.** `z_x` 전체를 스케일 다운해 모양을 유지한 채 세기만 낮춘다.
>
> 자세한 내용: [2026-08-28 합성 스파이크](../../experiments/2026-08-28-synthesis-spike/README.md)
...
6. ~~**src/dst를 뒤집으면 디코딩이 실패한다**~~ — **폐기 (2026-08-28).** 아래 6′ 로 대체
...
4번은 TPS 구현이 맞는지 가장 싸게 확인하는 방법이다.

### 6번을 폐기하는 이유와 대체 기준 (2026-08-28)

**뒤집어도 디코딩이 성공한다.** 20°·35°·50°·70° 전부 그랬다(제어점 좌표는 정규화 기준으로
0.008~0.104 만큼 확실히 벌어졌는데도). 왜곡을 만들 때 쓴 매핑에서 정답을 뽑으면
**라운드트립이 자기일관적**이라, 매핑이 틀렸든 방향이 뒤집혔든 되펴지면 읽힌다.

즉 방향 오류는 "정답이 틀리는" 오류가 아니라 **"의도한 것과 다른 왜곡이 만들어지는"** 오류다.
디코딩은 이 부류를 원리적으로 잡지 못하고, 학습 분포가 현장과 어긋나
**도메인 갭으로만** 드러난다. 대체 기준은 셋이다.

- **6′-a. 해석해 대조** — 원통에서 `cumsum(1/m)` 이 `r·arcsin((x−c)/r)` 와 0.5px 안에서 일치한다.
  실측: 정정판 0.24px / `cumsum(m)` 오류판 35.1px
- **6′-b. 단조성** — 제어점 `src` 가 `dst` 의 단조 역함수다. `m > 0` 이면 구조적으로 보장되므로
  깨졌다면 역보간을 빼먹은 것이다
- **6′-c. 펴진 이미지 일치** — 증강(회전·여백)을 넣은 뒤 대응장에서 다시 뽑은 제어점으로 펴면,
  증강 전에 편 결과와 픽셀 단위로 일치한다. 실측: 회전 2°에서 좌표를 갱신하면 평균 오차 3.4,
  갱신하지 않으면 59.5 — **17배로 갈린다.** 같은 조건에서 디코딩은 둘 다 성공한다

**증강 검증에 디코딩을 쓰면 안 된다.** 회전 25°까지 좌표가 틀려도 읽힌다.
...
| ~~**주름(fold)**~~ | ~~접힌 주름은 불연속이라 TPS가 표현하지 못할 수 있다~~ → **2026-08-28 해소.** 접힘은 표현된다(기울기를 `tanh` 로 전이시키면 된다). 못 표현하는 것은 **짧은 파장**이고, 한계는 가로 제어점 개수가 정한다 — 나이키스트 `λ ≥ 2W/(n_x−1)`. 4점이면 0.67W, 6점이면 0.40W. **격자를 4×3 → 6×3 으로 늘릴 것을 권고한다.** 세로는 `n_y=2` 로도 충분했다 | 해소 |
