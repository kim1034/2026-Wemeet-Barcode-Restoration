...
격자 계산이 지배적입니다. `tps_flow`를 1/4 크기로 계산하고 `cv2.resize`로 늘리는
방법을 검토합니다. 1/4 축소가 보정 품질을 떨어뜨리지 않는지는 별도 검증이 필요합니다.
...
보간법을 먼저 바꾸는 이유: 보정 비용이 약 24ms라 비교적 싸고, 얇은 바가 붙는 문제가 판독 실패의 흔한 원인입니다. 이 순서는 초기값이고 실측 후 `docs/decisions/0006`에 확정합니다.
...
| 기하 보정 (OpenCV) | 24 ms | **다시 합니다.** 3번 해도 72ms입니다 |
